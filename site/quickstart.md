...
In this quickstart you'll create a data dictionary for a real dataset — morphometric measurements of Alaskan sea otters — by asking an AI agent to do the heavy lifting, then checking and rendering the result yourself. It takes about ten minutes.

## Setup

1. [Install the `data-dict` CLI](install.md).
2. Download the data: [otters.parquet](https://github.com/hadley/otters) and the existing metadata file `seot_morphometricsReproStatus_ak_monson_metadata.html` from the same repo. Put both in a fresh directory.
...
2. Run `data-dict draft otters.parquet` to profile the data and generate a starting `data-dict.yaml`, with inferred types, observed ranges, and a `todo` note for everything only a human can decide.
3. Work through the todos, mining the metadata HTML for descriptions, units, and glossary terms — and asking you about anything it can't determine.

## No agent? No problem!

Call `data-dict draft otters.parquet` yourself then work through the file, filling in the `todo` items that data-dict created for you.
