...
| S06 | Inconsistent cardinality | E | The declared cardinality is inconsistent with the constraints on the joined columns (e.g. `one-to-many` whose "one" side is not `primary_key` or `unique`). A side uniquely identifies a row when its join columns include a `unique` column or cover the table's entire (possibly composite) primary key; one column of a composite primary key is not enough. |
